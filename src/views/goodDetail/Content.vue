<template>
    <section class="m-content" :class="{finish: options.order_status != 1}">
        <div class="order-status vh-between">
            <span class="name">{{options.cinema_name}}</span>
            <template v-if="options.order_status == '4'">
                <span v-if="options.snack_status == '4'" class="status">{{options.snack_status_name}}</span>
                <span v-else class="error">{{options.snack_status_name}}</span>
            </template>
        ...
        <div class="order-detail">
            <div class="goods">
                ...
                <div class="more vh-center" v-if="this.num > 0" @click="toggle"><span>{{text}}</span><i class="arrow-down" :class="{'arrow-top': isOpen}"></i></div>
            </div>
            <div class="split-line"></div>
            <div class="take-ticket" v-if="isShowQR()">
                <template v-if="options.print_code">
                    <div class="ticket-code vh-center">
                        <span>取票码</span><span class="code">{{options.print_code}}</span>
                    </div>
                    <div class="ticket-code vh-center">
                        <span>验证码</span><span class="code">{{options.verify_no}}</span>
                    </div>
...
<script>
    ...
    export default {
        ...
        data () {
            const num = this.options.snacks.length - 1;
            return {
                num: num,
                text: "展开剩余" + num + "条",
                isOpen: false,
                payFailMessage: "【该订单已支付失败，请重新尝试支付，或 重新购买。如有疑问，请与影城工作人员联系】",
                ticketFailMessage: "【该订单已支付成功，因票务系统故障导致未正常出票，涉及的费用会按照付款方式7个工作日内退回到您的账户。如有疑问，请与影城工作人员联系】",
                refundFailMessage: "【该订单退款失败，如有疑问，请与影城工作人员联系】",
            }
        },
        mounted () {
            if(this.options.print_code){
                const sp = viewport.rem() / 75;
                new QrCode(this.$refs.qrCode, {
                    text: this.options.print_code + "|" + this.options.verify_no,
                    width: 328 * sp,
                    height: 328 * sp,
        ...
        methods: {
            ...
            isShowQR() {
                ...
                return (opts.order_status == 4 && opts.snack_status == 4)
                    || opts.order_status == 6
            },
            ...
            showMessage() {
                ...
                return vm.options.order_status == '5' ? vm.payFailMessage
                    ...
                    : vm.options.order_status == '4' ? vm.ticketFailMessage
                    : ""
            }
        }
    }
...
<style lang="scss" scoped>
    ...
    .order-status{
        @include height(80px);
        @include radius(rem(5px) rem(5px) 0 0);
        padding: rem(0 20px);
        @include font-size(28px);
    }
    .order-detail{
        padding: rem(0 20px);
        @include radius(0 0 rem(5px) rem(5px));
    }
    .goods{
