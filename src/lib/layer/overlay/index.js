...
import { getZIndex, bind, fire } from 'lib/comp/bus'
...
const OverlayManager = {
    ...
    closeOverlay (touchMove) {
        ...
        if(this.overlays.length > 0){
            this.overlays[this.overlays.length-1].$el.removeEventListener("touchmove", this.preventDefault);
            this.overlays[this.overlays.length-1].show = false;
    ...
    changeOverlayStyle () {
        ...
        this.overlays[this.overlays.length - 1].setStyle(option);
    },
    isTop (zIndex) {
        if (this.options.length === 0) {
            return false;
